...
public class AuthService {
     ...
     */
    public void proceedAuth(User.AUTH_SOURCES auth_source, String code, HttpServletResponse response, HttpSession session) throws Exception {
        ...
        try {
            ...
            // 5. set jwt cookie <- Not Actually Needed
            if (auth_source == User.AUTH_SOURCES.GOOGLE) {
                ...
                if(!userRepository.queryUserExistence(userId, userDatabase)) {
                    ...
                    newUser.setUserId(userId);
                    if(!userAccount.getBoolean("profile_nickname_needs_agreement") && userAccount.has("profile")) newUser.setUserName(userAccount.getJSONObject("profile").getString("nickname"));
            ...
            // query user data
            User user = userRepository.queryUser(userId, userDatabase);

            // set session
            sessionService.initLoginSession(session, user.getUserId(), user.getUserName(), user.getPrivilege());
            //sessionService.addToSessionTable(session, true);
